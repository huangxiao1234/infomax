...
x_train,file_names = Tool.data_load("TrainingData/")
...
z_samples_repeat = RepeatVector(64 * 64)(z_samples)
z_samples_map = Reshape((64, 64, z_dim))(z_samples_repeat)
...
model_train.load_weights('cancer_model.weights')

# model_train.fit(x_train, epochs=10, batch_size=12)
# model_train.save_weights('cancer_model.weights')
...
df = pd.DataFrame(np.column_stack((file_names,zs)))
...
def sample_knn(path):
    ...
    imageio.imwrite(path+'cancer_l2.png', figure1)
    imageio.imwrite(path+'cancer_cos.png', figure2)
